...
const char * const BITCOIN_CONF_FILENAME = "sprint.conf";
const char * const BITCOIN_PID_FILENAME = "sprintd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sprint" is a composite category enabling all Sprint-related debug output
            if(ptrCategory->count(string("sprint"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SprintCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SprintCore
    // Mac: ~/Library/Application Support/SprintCore
    // Unix: ~/.sprintcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SprintCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SprintCore";
#else
    // Unix
    return pathRet / ".sprintcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sprint.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sprint.conf
